...
class Main
  ...
  def train_menu
    ...
    if train.class == PassengerTrain
      ...
      train.wagons[wagon_index-1].set_busy_volume(volume)
    end
    main_menu
  end
  ...
  def train_details(train)
    puts "Поезд: #{train.number}, тип: #{train.class}, количество вагонов: #{train.wagons.count}"
    if(train.class == PassengerTrain)
      train.each_wagon do |wagon, index|
        total_places = wagon.free_places + wagon.busy_places
        puts "Вагон №#{index}"
        puts "Всего мест: #{total_places}"
        puts "Свободных мест: #{wagon.free_places}"
        puts "Занятых мест: #{wagon.busy_places}"
      end
    else        
      train.each_wagon do |wagon, index|
        total_volume = wagon.free_volume + wagon.busy_volume
        puts "Вагон №#{index}"
